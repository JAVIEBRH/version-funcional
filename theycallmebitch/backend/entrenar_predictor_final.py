...
def calcular_factores_reales(df):
    ...
    try:
        ...
        pedidos_dict = pedidos_por_dia.to_dict()
        
        # Estadísticas generales
        valores = list(pedidos_dict.values())
        media_general = sum(valores) / len(valores) if valores else 0
        mediana_general = sorted(valores)[len(valores)//2] if valores else 0
        
        # Calcular desviación estándar
        if valores:
            varianza = sum((x - media_general) ** 2 for x in valores) / len(valores)
            desviacion_general = varianza ** 0.5
        else:
        ...
        for dia in range(7):
            ...
            if len(pedidos_dia) > 0:
                ...
                valores_dia = list(pedidos_por_dia_especifico.values)
                media_dia = sum(valores_dia) / len(valores_dia) if valores_dia else 0
        ...
        for mes in range(1, 13):
            ...
            if len(pedidos_mes) > 0:
                ...
                valores_mes = list(pedidos_por_mes_especifico.values)
                media_mes = sum(valores_mes) / len(valores_mes) if valores_mes else 0
        ...
        fechas_ordenadas = sorted(pedidos_dict.keys())
        ...
        if len(fechas_ordenadas) >= 14:  # Al menos 2 semanas
            ...
            try:
                promedio_primera = sum(pedidos_dict[fecha] for fecha in primera_semana) / 7
                promedio_ultima = sum(pedidos_dict[fecha] for fecha in ultima_semana) / 7
        ...
        return {
            ...
            "total_dias_analizados": len(pedidos_dict),
